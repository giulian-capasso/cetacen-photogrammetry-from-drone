...
lidar_filter <- function(x, window_size, lower_limit, upper_limit) {
  result <- x
  for (i in 1:length(x)) {
    # Omit borders
    if (!is.na(x[i])) {
      ...
      median_value <- median(x[start:end], na.rm = TRUE)
      if (x[i] > upper_limit || x[i] < lower_limit) {
...
# Applica il filtro personalizzato alla colonna con window_size = 10, lower_limit = 2, upper_limit = 50
FLY285_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter(
  FLY285_REC_MOT_LD_1.2$laser_altitude_m,
  window_size = 10,
  lower_limit = 2,
  upper_limit = 50
)

clean_285 <- ggplot(FLY285_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  ...
  labs(title = "4Confronto tra Altezza Lidar originale, pulita e Barometrica",
       y = "Height",
       x = "Time") +
  theme_minimal() +
...
## OPTION 2 ## (more accurete with bar)
  ...
  for (i in 1:length(lidar)) {
    ...
    if (!is.na(lidar[i]) && i > omit_first_n && i <= (length(lidar) - omit_last_n)) {
      ...
      if (lidar[i] > 60 || lidar[i] < 2) {
        ...
        valid_differences <- differences[complete.cases(differences) & differences >= lower_limit & differences <= upper_limit]
...
# bella funzione 
ill_wB <- function(lidar_col, baro_col, window_size, omit_first_n = 0, omit_last_n = 0) {
...
FLY421_REC_MOT_LD_1.2$laser_altitude_m_cleaned4 <- fill_wB(
...
#plot_263, plot_264, plot_269, plot_270 NO LOG 
#plot_285, plot_288 a terra = non considerare
...
plot_297 <- ggplot(FLY297_REC_MOT_LD_1.2) +
  geom_line(aes(x = GPS.dateTimeStamp, y = laser_altitude_m_cleaned, color = "LIDAR"), size = 0.7, linetype = "solid") +
  geom_line(aes(x = GPS.dateTimeStamp, y = `osd_data:relativeHeight[meters]`, color = "BAR"), size = 0.7, linetype = "solid") +
...
# Applica il Lidar Filter con window_size = 5, lower_limit = 2, upper_limit = 50
FLY297_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY297_REC_MOT_LD_1.2$laser_altitude_m,
  FLY297_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 150,
...
# Lidar Cleaning 
FLY298_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY298_REC_MOT_LD_1.2$laser_altitude_m,
  FLY298_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 50,
...
clean_298 <- ggplot(FLY298_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  ...
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
  labs(title = "2022-06-16 06:19:01 (FLY_298 LOG0028)",
       y = "Height",
       x = "Time") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "darkorange", "darkgray")) +
...
# Lidar Cleaning 
FLY299_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
...
plot_304 <- ggplot(FLY304_REC_MOT_LD_1.2) +
  geom_line(aes(x = GPS.dateTimeStamp, y = laser_altitude_m, color = "LIDAR"), size = 0.7, linetype = "solid") +
  geom_line(aes(x = GPS.dateTimeStamp, y = `osd_data:relativeHeight[meters]`, color = "BAR"), size = 0.7, linetype = "solid") +
  labs(title = "2022-06-30 08:15:31 (FLY_304 LOG_0030)", x = "Time", y = "Height (m)") +
  theme_minimal() +
  theme(legend.position = c(0.9, 0.9),
        legend.box = "horizontal", legend.background = element_rect(color = "gray", fill = "white"),
        legend.margin = margin(t = -6, r = 6, b = -1.5, l = 6), axis.text = element_text(size = 8),
...
# Lidar Cleaning 
FLY304_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY304_REC_MOT_LD_1.2$laser_altitude_m,
  FLY304_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 20,
...
FLY306_REC_MOT_ <- read_csv("Desktop/Matched 1/BAR/FLY306_REC_MOT.csv")
# Import LOG 
...
# Lidar Cleaning 
FLY306_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY306_REC_MOT_LD_1.2$laser_altitude_m,
  FLY306_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 20,
...
# Lidar Cleaning 
FLY307_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY307_REC_MOT_LD_1.2$laser_altitude_m,
  FLY307_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 3,
...
clean_307 <- ggplot(FLY307_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY310_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY310_REC_MOT_LD_1.2$laser_altitude_m,
  FLY310_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 3,
...
# Lidar Cleaning 
FLY311_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY311_REC_MOT_LD_1.2$laser_altitude_m,
  FLY311_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 3,
...
clean_311 <- ggplot(FLY311_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  ...
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
  labs(title = "2022-07-10 12:34:10 (FLY_311 LOG_0035)",
       y = "Height",
       x = "Time") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "darkorange", "darkgray")) +
...
# Lidar Cleaning 
FLY312_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY312_REC_MOT_LD_1.2$laser_altitude_m,
  FLY312_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 3,
...
clean_312 <- ggplot(FLY312_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
  labs(title = "2022-07-12 08:54:10 (FLY_312 LOG_0036)",
       y = "Height",
       x = "Time") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "darkorange", "darkgray")) +
...
# Lidar Cleaning 
FLY313_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY313_REC_MOT_LD_1.2$laser_altitude_m,
  FLY313_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY315_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY315_REC_MOT_LD_1.2$laser_altitude_m,
  FLY315_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY316_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY316_REC_MOT_LD_1.2$laser_altitude_m,
  FLY316_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY317_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY317_REC_MOT_LD_1.2$laser_altitude_m,
  FLY317_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
clean_317 <- ggplot(FLY317_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY318_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY318_REC_MOT_LD_1.2$laser_altitude_m,
  FLY318_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY319_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY319_REC_MOT_LD_1.2$laser_altitude_m,
  FLY319_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY323_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY323_REC_MOT_LD_1.2$laser_altitude_m,
  FLY323_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY327_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY327_REC_MOT_LD_1.2$laser_altitude_m,
  FLY327_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY330_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY330_REC_MOT_LD_1.2$laser_altitude_m,
  FLY330_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY332_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY332_REC_MOT_LD_1.2$laser_altitude_m,
  FLY332_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY352_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY352_REC_MOT_LD_1.2$laser_altitude_m,
  FLY352_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY353_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY353_REC_MOT_LD_1.2$laser_altitude_m,
  FLY353_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY354_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY354_REC_MOT_LD_1.2$laser_altitude_m,
  FLY354_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY368_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY368_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY368_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY371_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY371_REC_MOT_LD_1.2$laser_altitude_m,
  FLY371_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 10,
...
# Lidar Cleaning 
FLY372_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY372_REC_MOT_LD_1.2$laser_altitude_m,
  FLY372_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
clean_372 <- ggplot(FLY372_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY374_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY374_REC_MOT_LD_1.2$laser_altitude_m,
  FLY374_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY382_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY382_REC_MOT_LD_1.2$laser_altitude_m,
  FLY382_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY383_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY383_REC_MOT_LD_1.2$laser_altitude_m,
  FLY383_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
clean_383 <- ggplot(FLY383_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  ...
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
  labs(title = "2022-10-18 10:04:40 (FLY_383 LOG_0080)",
       y = "Height",
       x = "Time") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "darkorange", "darkgray")) +
...
# Lidar Cleaning 
FLY384_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY384_REC_MOT_LD_1.2$laser_altitude_m,
  FLY384_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
clean_384 <- ggplot(FLY384_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY387_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY387_REC_MOT_LD_1.2$laser_altitude_m,
  FLY387_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY388_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY388_REC_MOT_LD_1.2$laser_altitude_m,
  FLY388_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
clean_388 <- ggplot(FLY388_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY389_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY389_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY389_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Spikes cleaning - NO SPIKES ON THIS FLIGHT 
...
# Lidar Cleaning 
FLY396_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY396_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY396_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY400_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY400_REC_MOT_LD_1.2$laser_altitude_m,
  FLY400_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY401_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY401_REC_MOT_LD_1.2$laser_altitude_m,
  FLY401_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY402_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY402_REC_MOT_LD_1.2$laser_altitude_m,
  FLY402_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
clean_411 <- ggplot(FLY411_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Clean "0002-11-30"
FLY412_REC_MOT_LD_1.2_clean <- subset(FLY412_REC_MOT_LD_1.2_clean, !grepl("0002-11-30", GPS.dateTimeStamp))
# Plot 
...
# Lidar Cleaning 
FLY412_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY412_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY412_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY414_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY414_REC_MOT_LD_1.2$laser_altitude_m,
  FLY414_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY417_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY417_REC_MOT_LD_1.2$laser_altitude_m,
  FLY417_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 30,
  upper_limit = 60,
  omit_first_n = 0,
...
# Lidar Cleaning 
FLY420_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY420_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY420_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 1,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
FLY420_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned2 <- lidar_filter3(
  FLY420_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned,
  FLY420_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 50,
  lower_limit = 0,
  upper_limit = 80,
...
clean_420 <- ggplot(FLY420_REC_MOT_LD_1.2_clean, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned2, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
clean_421 <- ggplot(FLY421_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar difettoso. Bloccato a 2 cm 
...
# Lidar Cleaning 
FLY423_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY423_REC_MOT_LD_1.2$laser_altitude_m,
  FLY423_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 1,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_423 <- ggplot(FLY423_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
  labs(title = "2023-06-18 05:37:01 (FLY_423 LOG_112)",
       y = "Height",
       x = "Time") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "darkorange", "darkgray")) +
...
plot_424_clean <- ggplot(FLY424_REC_MOT_LD_1.2) +
  geom_line(aes(x = GPS.dateTimeStamp, y = laser_altitude_m_cleaned, color = "LIDAR"), size = 0.7, linetype = "solid") +
  geom_line(aes(x = GPS.dateTimeStamp, y = `osd_data:relativeHeight[meters]`, color = "BAR"), size = 0.7, linetype = "solid") +
  labs(title = "2023-06-18 09:02:01 (FLY_424)", x = "Time", y = "Height (m)") +
  theme_minimal() +
  ...
  scale_colour_manual(name = NULL, values = c("LIDAR" = "darkorange", "BAR" = "blue")) +
  theme(plot.title = element_text(size = 10, face = "bold")) 
plot_424_clean  <- plot_424_clean  + ylim(0, 130)
plot_424 + plot_424_clean

# Lidar Cleaning 
FLY424_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY424_REC_MOT_LD_1.2$laser_altitude_m,
  FLY424_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 1,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_424 <- ggplot(FLY424_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY425_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY425_REC_MOT_LD_1.2$laser_altitude_m,
  FLY425_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 1,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_425 <- ggplot(FLY425_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY427_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY427_REC_MOT_LD_1.2$laser_altitude_m,
  FLY427_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 1,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
FLY427_REC_MOT_LD_1.2$laser_altitude_m_cleaned[468] <- NA

clean_427 <- ggplot(FLY427_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY429_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY429_REC_MOT_LD_1.2$laser_altitude_m,
  FLY429_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 1,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_429 <- ggplot(FLY429_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY430_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY430_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY430_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_430 <- ggplot(FLY430_REC_MOT_LD_1.2_clean, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY431_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY431_REC_MOT_LD_1.2$laser_altitude_m,
  FLY431_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_431 <- ggplot(FLY431_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY434_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY434_REC_MOT_LD_1.2$laser_altitude_m,
  FLY434_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_434 <- ggplot(FLY434_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY436_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY436_REC_MOT_LD_1.2$laser_altitude_m,
  FLY436_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
FLY436_REC_MOT_LD_1.2$laser_altitude_m_cleaned[313:366] <- NA

clean_436 <- ggplot(FLY436_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY437_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY437_REC_MOT_LD_1.2$laser_altitude_m,
  FLY437_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 10,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_437 <- ggplot(FLY437_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY438_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY438_REC_MOT_LD_1.2$laser_altitude_m,
  FLY438_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_438 <- ggplot(FLY438_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY440_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY440_REC_MOT_LD_1.2$laser_altitude_m,
  FLY440_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_440 <- ggplot(FLY440_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
#Spikes cleaning
FLY446_REC_MOT_LD_1.2$laser_altitude_m_cleaned3 <- lidar_filter3(
...
# Lidar Cleaning 
FLY447_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY447_REC_MOT_LD_1.2$laser_altitude_m,
  FLY447_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_447 <- ggplot(FLY447_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY452_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY452_REC_MOT_LD_1.2$laser_altitude_m,
  FLY452_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 0,
  upper_limit = 80,
  omit_first_n = 0,
...
FLY452_REC_MOT_LD_1.2$laser_altitude_m_cleaned[231:232] <- NA

clean_452 <- ggplot(FLY452_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY456_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY456_REC_MOT_LD_1.2$laser_altitude_m,
  FLY456_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 20,
...
clean_456 <- ggplot(FLY456_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY457_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY457_REC_MOT_LD_1.2$laser_altitude_m,
  FLY457_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 20,
...
clean_457 <- ggplot(FLY457_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY458_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY458_REC_MOT_LD_1.2$laser_altitude_m,
  FLY458_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 50,
  upper_limit = 80,
  omit_first_n = 600,
...
clean_458 <- ggplot(FLY458_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY459_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY459_REC_MOT_LD_1.2$laser_altitude_m,
  FLY459_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 50,
  upper_limit = 80,
  omit_first_n = 60,
...
clean_459 <- ggplot(FLY459_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
  labs(title = "2023-08-09 08:59:00 (FLY_459 LOG_151)",
       y = "Height",
       x = "Time") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "darkorange", "darkgray")) +
...
# Lidar Cleaning 
FLY461_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY461_REC_MOT_LD_1.2$laser_altitude_m,
  FLY461_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 50,
  upper_limit = 80,
  omit_first_n = 60,
...
clean_461 <- ggplot(FLY461_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY462_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY462_REC_MOT_LD_1.2$laser_altitude_m,
  FLY462_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_462 <- ggplot(FLY462_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY463_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY463_REC_MOT_LD_1.2$laser_altitude_m,
  FLY463_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 150,
...
clean_463 <- ggplot(FLY463_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY464_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY464_REC_MOT_LD_1.2$laser_altitude_m,
  FLY464_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 50,
...
clean_464 <- ggplot(FLY464_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY466_REC_MOT_LD_1.2_clean$laser_altitude_m_cleaned <- lidar_filter3(
  FLY466_REC_MOT_LD_1.2_clean$laser_altitude_m,
  FLY466_REC_MOT_LD_1.2_clean$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_466 <- ggplot(FLY466_REC_MOT_LD_1.2_clean, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY472_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY472_REC_MOT_LD_1.2$laser_altitude_m,
  FLY472_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 80,
  omit_first_n = 0,
...
clean_472 <- ggplot(FLY472_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY475_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY475_REC_MOT_LD_1.2$laser_altitude_m,
  FLY475_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 50,
  omit_first_n = 120,
...
clean_475 <- ggplot(FLY475_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY477_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY477_REC_MOT_LD_1.2$laser_altitude_m,
  FLY477_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 50,
  omit_first_n = 120,
...
FLY477_REC_MOT_LD_1.2$laser_altitude_m_cleaned[417:419] <- NA

clean_477 <- ggplot(FLY477_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY478_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY478_REC_MOT_LD_1.2$laser_altitude_m,
  FLY478_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 50,
  omit_first_n = 120,
...
clean_478 <- ggplot(FLY478_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY484_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY484_REC_MOT_LD_1.2$laser_altitude_m,
  FLY484_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 50,
  omit_first_n = 10,
...
clean_484 <- ggplot(FLY484_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
# Lidar Cleaning 
FLY487_REC_MOT_LD_1.2$laser_altitude_m_cleaned <- lidar_filter3(
  FLY487_REC_MOT_LD_1.2$laser_altitude_m,
  FLY487_REC_MOT_LD_1.2$`osd_data:relativeHeight[meters]`,
  window_size = 5,
  lower_limit = 10,
  upper_limit = 50,
  omit_first_n = 10,
...
clean_487 <- ggplot(FLY487_REC_MOT_LD_1.2, aes(x = GPS.dateTimeStamp)) +
  geom_line(aes(y = laser_altitude_m, color = "Raw Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = laser_altitude_m_cleaned, color = "Cleaned Lidar"), linetype = "solid", size = 0.5) +
  geom_line(aes(y = `osd_data:relativeHeight[meters]`, color = "BAR"), linetype = "solid", size = 0.5) +
...
setwd("~/Desktop")
pdf_file <- "cleaned_combined.pdf"
pdf(pdf_file, width = 8.5, height = 11)
clean_plot_list <- list()
# Lista dei nomi dei plot
clean_plot_names <- c("clean_297", "clean_298", "clean_299", "clean_304" ,"clean_306", "clean_307", "clean_310", 
                      "clean_311", "clean_312", "clean_313", "clean_315", "clean_316", "clean_317", "clean_318",
                      "clean_319", "clean_323", "clean_327", "clean_330", "clean_332", "clean_352", "clean_353", 
                      "clean_354", "clean_365", "clean_368", "clean_371", "clean_372", "clean_374", "clean_382",
...
plot_list <- lapply(clean_plot_names, function(name) get(name))
...
pdf("plot_cleaned.pdf", width = 16, height = 9)
...
for (page in 1:num_pages) {
  ...
  cat("Page:", page, "| Start index:", start_index, "| End index:", end_index, "\n")
